            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold flex items-center gap-2">
                  <Bot className="h-5 w-5 text-[#9B5BDB]" />
                  {t("coach.title")}
                </h1>
      ...
      <main className="flex-1 container max-w-4xl mx-auto px-4 py-6 flex flex-col">
        {loadingMessages ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
            <div className="w-20 h-20 rounded-2xl bg-[#9B5BDB] flex items-center justify-center mb-6 shadow-xl animate-scale-in">
            ...
            <Card className="bg-[#9B5BDB]/10 border-[#9B5BDB]/30 mb-8 max-w-md">
              <CardContent className="py-3 px-4">
                <p className="text-sm text-foreground/80 italic flex items-center gap-2"><Heart className="h-4 w-4 text-[#9B5BDB] flex-shrink-0" />{randomTip}</p>
              </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-2 gap-3 w-full max-w-md">
              {quickActions.map((action) => (
                <button key={action.id} onClick={() => handleQuickAction(action.prompt)} disabled={isLoading}
                  className="flex items-center gap-2 p-4 rounded-xl bg-muted/50 hover:bg-muted border border-border/50 hover:border-[#9B5BDB]/40 transition-all text-left group">
                  <div className="w-9 h-9 rounded-lg bg-[#9B5BDB]/15 flex items-center justify-center group-hover:bg-[#9B5BDB]/25 transition-colors"><action.icon className="h-4 w-4 text-[#9B5BDB]" /></div>
            ...
            <div className="flex items-center gap-4 mt-8 text-muted-foreground">
              ...
              <div className="flex items-center gap-1.5 text-sm"><Target className="h-4 w-4 text-[#9B5BDB]" /><span>{t("coach.pillars.build_habits")}</span></div>
          ...
          <>
              ...
              <div className="space-y-4 pb-4">
                {messages.map((msg, idx) => (
                  <div key={idx} className={`flex gap-3 animate-fade-in ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    {msg.role === "assistant" && <div className="w-9 h-9 rounded-xl bg-[#9B5BDB] flex items-center justify-center flex-shrink-0 shadow-md"><Bot className="h-5 w-5 text-white" /></div>}
                    <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${msg.role === "user" ? "bg-[#9B5BDB] text-white rounded-br-md font-medium" : "bg-muted rounded-bl-md"}`}>
                ...
                {isLoading && messages[messages.length - 1]?.role === "user" && (
                  <div className="flex gap-3 animate-fade-in">
                    <div className="w-9 h-9 rounded-xl bg-[#9B5BDB] flex items-center justify-center shadow-md"><Bot className="h-5 w-5 text-white" /></div>
                    <div className="bg-muted rounded-2xl rounded-bl-md px-4 py-3">
                      <div className="flex gap-1">
                        <span className="w-2 h-2 bg-[#9B5BDB]/60 rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
                        <span className="w-2 h-2 bg-[#9B5BDB]/60 rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
                        <span className="w-2 h-2 bg-[#9B5BDB]/60 rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="flex gap-2 py-3 overflow-x-auto scrollbar-hide">
              {quickActions.map((action) => (
                <Badge key={action.id} variant="secondary" className="cursor-pointer hover:bg-[#9B5BDB]/15 transition-colors whitespace-nowrap px-3 py-1.5" onClick={() => handleQuickAction(action.prompt)}>
                  <action.icon className="h-3 w-3 mr-1.5 text-[#9B5BDB]" />{action.label}
                </Badge>
              ))}
            </div>
          </>
        ...
        <div className="pt-4 border-t border-border mt-auto safe-area-bottom">
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-3">
            <Input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} placeholder={t("coach.input_placeholder")} disabled={isLoading} className="flex-1 h-12 rounded-xl bg-muted/50 border-border/50 focus:border-[#9B5BDB]/50" />
            <Button type="submit" size="icon" disabled={isLoading || !input.trim()} className="h-12 w-12 rounded-xl bg-[#9B5BDB] hover:bg-[#8b4bca] text-white">
