...
const CHALLENGE_CATEGORY_VALUES = ["digital_detox", "mental_reset", "porn_detox", "general"] as const;
...
export default function CreateChallengeModal({ open, onOpenChange, onSuccess }: CreateChallengeModalProps) {
  ...
  const { isPremium, canCreateChallenge, challengesRemaining, totalChallengeCount, refetch: refetchLimits } =
    usePremiumLimits();
  ...
  useEffect(() => {
    if (open && !canCreateChallenge) setShowPaywall(true);
  }, [open, canCreateChallenge]);

  const addStep = () => {
    if (dailySteps.length < 7) setDailySteps([...dailySteps, ""]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("detox_challenges").insert({
        ...
        category,
        daily_steps: filteredSteps.length > 0 ? filteredSteps : null,
        start_date: today,
      });
      if (error) throw error;
      await supabase.from("profiles").update({ total_challenges_created: totalChallengeCount + 1 } as { total_challenges_created: number }).eq("user_id", user.id);
      toast({
        title: t("create_challenge.challenge_started"),
        description: t("create_challenge.challenge_started_desc", { days: duration }),
      });
      resetForm();
      refetchLimits();
      onOpenChange(false);
    ...
    <>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {challengesRemaining === 0
                    ? t("create_challenge.free_challenge_used")
                    : t("create_challenge.free_challenge_available", { remaining: challengesRemaining })}
                </span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={t("create_challenge.challenge_placeholder")}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select value={category} onValueChange={setCategory}>
                  ...
                  <SelectContent>
                    {CHALLENGE_CATEGORY_VALUES.map((val) => (
                      <SelectItem key={val} value={val}>
                        {t(`challenges.categories.${val}`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                  ...
                  <SelectContent>
                    {DURATION_KEYS.map((opt) => (
                      <SelectItem key={opt.value} value={opt.value.toString()}>
                        {t(`create_challenge.durations.${opt.key}_days`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder={t("create_challenge.description_placeholder")}
                ...
                {dailySteps.map((step, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      placeholder={t("create_challenge.step_placeholder", { n: index + 1 })}
                      className="rounded-xl flex-1"
                      maxLength={200}
                    />
                    {dailySteps.length > 1 && (
                      <button type="button" onClick={() => removeStep(index)} className="p-1 hover:bg-muted rounded-full transition-colors">
      ...
      <PaywallModal
        ...
        onOpenChange={(isOpen) => {
          ...
          if (!isOpen && !canCreateChallenge) onOpenChange(false);
        }}
