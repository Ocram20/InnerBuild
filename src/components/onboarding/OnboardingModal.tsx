...
export default function OnboardingModal({ open, onComplete, onSkip }: OnboardingModalProps) {
  ...
  const submitOnboarding = async (prefs: CategoryPreferences) => {
    setSubmitting(true);
    try {
      await onComplete(selectedFocus, prefs);
      toast({
  ...
  const handlePaywallClose = async (isOpen: boolean) => {
    if (!isOpen) {
      setShowPaywall(false);
      // Save free preferences only if paywall dismissed
      const freeOnlyPrefs: CategoryPreferences = {
        ...toolsState,
        "the-forge": false,
        "trigger-tracking": false,
  ...
  return (
    <>
      <Dialog open={open && !showPaywall} onOpenChange={() => {}}>
        <DialogContent className="max-w-lg p-0 overflow-hidden bg-card text-foreground border border-border/80 dark:border-white/10 shadow-2xl rounded-[28px] [&>button]:hidden transition-colors duration-200">
          {/* Header Illustration & Navigation Area */}
          <div className="relative pt-7 pb-5 px-6 bg-gradient-to-b from-emerald-500/15 via-emerald-500/5 to-transparent text-center border-b border-border/40 dark:border-white/5">
            {/* Top Row: Step badge & Skip button */}
            <div className="flex items-center justify-between mb-4">
              <span className="bg-muted/80 dark:bg-white/5 border border-border/60 dark:border-white/10 text-muted-foreground text-[11px] font-medium px-3 py-1 rounded-full">
              ...
              <button
                onClick={handleSkip}
                disabled={submitting}
                className="text-xs font-medium text-muted-foreground hover:text-foreground transition-colors px-3 py-1 rounded-full hover:bg-muted dark:hover:bg-white/5"
              >
            ...
            <div className="w-14 h-14 rounded-full bg-emerald-500/10 dark:bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center mx-auto mb-3 shadow-sm">
              <Leaf className="h-7 w-7 text-emerald-600 dark:text-emerald-400" />
            </div>

            {/* Progress Bar */}
            <div className="w-full max-w-xs mx-auto h-1.5 bg-muted dark:bg-white/10 rounded-full overflow-hidden">
              <div
          ...
          <div className="p-6 md:p-8 space-y-6 max-h-[70vh] overflow-y-auto">
            {step === 1 ? (
              ...
              <div className="space-y-6 animate-fade-in">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? "bg-emerald-500/10 border-emerald-500/50 dark:border-emerald-500/40 text-foreground shadow-sm"
                            : "bg-muted/30 dark:bg-card/40 border-border/60 hover:bg-muted/70 dark:hover:bg-card/80 hover:border-border"
                        )}
                      >
                        <div className="w-10 h-10 rounded-full bg-emerald-500/10 dark:bg-emerald-500/15 border border-emerald-500/20 flex items-center justify-center shrink-0">
                        ...
                        <div
                          className={cn(
                            ...
                            isSelected
                              ...
                              : "border-border/80 dark:border-border/60 bg-transparent"
                          )}
                        >
                          {isSelected && <Check className="h-3.5 w-3.5 stroke-[3] text-white dark:text-slate-950" />}
                        </div>
                      </div>
                    );
                  })}
                ...
                <Button
                  onClick={() => setStep(2)}
                  className="w-full h-12 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-500 dark:to-teal-500 text-white dark:text-slate-950 font-bold text-base hover:from-emerald-700 hover:to-teal-700 dark:hover:from-emerald-400 dark:hover:to-teal-400 active:scale-[0.98] transition-all shadow-md"
                >
              ...
              <div className="space-y-6 animate-fade-in">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-2">
                          ...
                          {tool.isPremium && (
                            <Badge className="bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 text-[10px] font-bold px-2 py-0.5">
                        ...
                        <Checkbox
                          ...
                          className="h-5 w-5 border-border/80 dark:border-border/60 data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500 data-[state=checked]:text-white dark:data-[state=checked]:text-slate-950"
                        />
                      </div>
                    );
                  })}
                ...
                {hasSelectedPremium && (
                  <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-800 dark:text-emerald-300 flex items-start gap-2 animate-slide-up">
                    <Crown className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                ...
                <div className="space-y-1.5 pt-1">
                  <Button
                    ...
                    disabled={submitting}
                    className="w-full h-11 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-500 dark:to-teal-500 text-white dark:text-slate-950 font-bold text-sm hover:from-emerald-700 hover:to-teal-700 dark:hover:from-emerald-400 dark:hover:to-teal-400 active:scale-[0.98] transition-all"
                  >
